...
abstract class SingleUseCase<T, Params>(
    ...
    abstract fun buildObservable(params: Params?): Single<T>

    override fun execute(callback: DisposableSingleObserver<T>, params: Params?) {
        val observable: Single<T> = initObservable(params)
    ...
    private fun initObservable(): Single<T> {
        return initObservable(null)
    }

    private fun initObservable(params: Params?): Single<T> {
